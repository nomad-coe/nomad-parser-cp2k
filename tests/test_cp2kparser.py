...
def test_single_point(parser):
    ...
    assert sec_method.basis_set[0].atom_centered.name == 'DZVP-GTH-PADE'
    assert sec_method.basis_set[0].cell_dependent.planewave_cutoff.magnitude == approx(6.53961708e-16)
...
def test_molecular_dynamics(parser):
    ...
    assert sec_sccs[7].thermodynamics.temperature.magnitude == approx(218.299664775)
    assert sec_sccs[9].thermodynamics.kinetic_energy.magnitude == approx(2.4094966278264588e-20)
